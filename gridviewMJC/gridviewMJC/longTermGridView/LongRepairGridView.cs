...
namespace coms.COMSK.ui.common
{
    public class LongRepairGridView<T> : DataGridView
    {
        ...
        private void OnCellPainting(object sender, DataGridViewCellPaintingEventArgs e)
        {
            ...
            bool isOwner = (owner.Row == e.RowIndex && owner.Col == e.ColumnIndex);
            if (!isOwner)
            {
                e.Handled = true; // covered cell
                return;
            }
            ...
            using (var backBrush = new SolidBrush(e.CellStyle.BackColor))
                e.Graphics.FillRectangle(backBrush, mergedRect);

            using (var pen = new Pen(GridColor))
                e.Graphics.DrawRectangle(pen, mergedRect.X, mergedRect.Y, mergedRect.Width - 1, mergedRect.Height - 1);
            ...
            TextRenderer.DrawText(
                ...
                Rectangle.Inflate(mergedRect, -2, -2),
        ...
        private void OnMouseMoveDrag(object sender, MouseEventArgs e)
        {
            ...
            var hit = HitTest(e.X, e.Y);
            if (hit.Type != DataGridViewHitTestType.Cell) return;
            if (hit.RowIndex != _dragStart.Row) return;
        ...
        private void OnMouseUpDrag(object sender, MouseEventArgs e)
        {
            ...
            if (_dragStart.Row >= 0 && _dragEnd.Row == _dragStart.Row)
            {
                DataGridViewColumn from = Columns[_dragStart.Col];
                DataGridViewColumn to = Columns[_dragEnd.Col];
