...
namespace coms.COMSK.common
{
	...
	public static class COMSKCommon
	{
		...
		public static string[] GetSelectedTextAsArrayOfCheckedCombobox(coms.COMMON.ui.CheckedComboBox cmbBox)
		{
			string selectedValues = cmbBox.GetCheckedItemsText();
		...
		public static string Get30thColName(int accountStartPeriodIdx)
        {
			var colIdx = accountStartPeriodIdx + 29;  // 固定60年間ので半分がaccountStartPeriodIdx + 29
			return "bgcolValue" + colIdx.ToString();
		}
		...
		public static void RemoveYearlyCol(DataGridView grid)
		{
			List<DataGridViewColumn> tmpCols = new List<DataGridViewColumn>();
			foreach(DataGridViewColumn col in grid.Columns)
            {
				if (col is DataGridViewTextBoxColumn)
                {
					if ((col as DataGridViewTextBoxColumn).Tag?.ToString() == COMSKCommon.TAG_DRAGGABLE_CELL)
                    {
						tmpCols.Add(col);
					}
                }
            }
			foreach(var col in tmpCols)
            {
