...
namespace coms.COMMON.ui
{
    ...
    public partial class DataGridViewEx : AdvancedDataGridView
    {
        ...
        private static readonly Color _linkColor = ColorTranslator.FromHtml("#7D7D7D");
        #endregion
        ...
        //private IList<object> _originalListData = new List<object>();
        //List<T>としてBindingSourceに入れる
        ...
        // フィルターポップアップにある並び替えメニューの非活性にしたい列を指定する
        public HashSet<string> DisabledMenuSortColumns { get; set; }
            = new HashSet<string>(StringComparer.OrdinalIgnoreCase);

        //並び替えしない列名を格納する
        public HashSet<string> DisabledSortColumns { get; set; }
            = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        public bool DisabledAllMenuSort { get; set; } = false;
        ...
        public bool DisabledCustomFilterMenuAllColumns { get; set; } = false;
        private bool _suppressSortOnce = false;
        private bool _isResizingColumn = false;
        // -------
        // IsSortHandledByChild:
        // 子コントロールがソートを処理する場合、親のDataGridViewExはソートイベントを無視します。
        // これにより、子コントロールが独自のソートロジックを実装できるようになります。
        public bool IsSortHandledByChild { get; set; } 
        // -------
        ...
        // 日付、数値の列設定を格納する(例：値を消しフォーカスアウトする際に値クリアする、戻す、0表示する等です。)
        private Dictionary<string, ColumnConfig> _externalConfigs = new Dictionary<string, ColumnConfig>();

        //本来編集出来る・出来ないGridを設定する
        private GridMode _mode;
        //現在Gridで表示しているデータは絞り込みしたLINQのクエリ
        private IQueryable _query;
        //データソースに入れたCLASS<T>を格納する
        private Type _itemType;
        //Gridのデータソースにデータを格納するBindingSourceです。
        ...
        /// <summary>
        /// List<T>がなくならない為 Gridにデータを入れたら、この関数を使ってください。
        /// </summary>
        ...
        public void SetData<T>(IList<T> data, GridMode mode = GridMode.ReadOnly)
        {
            ...
            {
                _originalListData = data.ToList(); // fallback
            }
            //_bindingSource.DataSource = new BindingList<T>(_originalListData);
            //ConfigureMode();
        ...
        {
            //if (_mode != GridMode.Editable) return;
            if (item == null) return;
            ...
            {
                throw new InvalidOperationException($"Item must be of type {_itemType.Name}");
            }
        ...
        public void UpdateItem(object item)
        {
            ...
            if (index != -1)
            {
                ...
                // reload datasource and refesh grid
                ApplyListFilterAndSort();
            }
        }
        ...
        {
            if (_clearFilterButton == null || _clearFilterButton.Visible == false) return;
            int x = 5;
        ...
        private void RebuildGrouping()
        {
            ...
            var items = ExtractEnumerableItemsFromDataSource();
            if (items == null)
                return;
        ...
        {
            object source = _bindingSource.DataSource;
            ...
            if (source is DataTable dt)
                return dt.AsEnumerable().Cast<object>().ToList();

            return new List<object>();
        }

        public List<T> GetDataSourceByList<T>() where T : class
        {
            List<object> myList = this.ExtractEnumerableItemsFromDataSource(); 
            return myList.Cast<T>().ToList();
        }

        /// <summary>
        /// Get List<T> from bindingSouce 
        /// </summary>
        /// <typeparam name="T"></typeparam>
        /// <returns></returns>
        public List<T> GetListDataFromBindingSource<T>()
        {
            _bindingSource.EndEdit();
            if (_bindingSource.DataSource == null) return new List<T>();
            if (_bindingSource.DataSource is List<T> originalList)
            {
                return originalList; 
            }
            if (_bindingSource.DataSource is IEnumerable dataSource)
            {
                return dataSource.OfType<T>().ToList();
            }

            return new List<T>();
            //var dataSource = _bindingSource.DataSource as IEnumerable;
            //return dataSource?.Cast<T>().ToList() ?? new List<T>();
        }


        public void AddItemtoDataSource(object rowItem)
        {
        ...
        public void RemoveItemfromDataSource(object rowItem)
        {
            if (rowItem == null) return;
        ...
        private void RefreshDisplayRowsUI()
        {
            ...
            foreach (var r in _displayRows)
            {
                ...
                {
                    // Add child row in the group
                    int idx = this.Rows.Add();
                    var row = this.Rows[idx];
                    row.Tag = r; //push data of row into tag
        ...
        {
            //disabled sort and filter for button columns etc...
            //DisabledFilterColumns = new HashSet<string>(StringComparer.OrdinalIgnoreCase);
        ...
        private void DataGridViewEx_FilterStringChanged(object sender, EventArgs e)
        {
            ...
            {
                if (this.DisabledFilterAll || IsFilterDisabled(fld))
                {
        ...
        private void ApplyListFilterAndSort()
        {
            ...
            try
            {
                ...
                if(result != null && result.Count > 0)
                    ...
                    _bindingSource.DataSource = _itemType;
        ...
        private string ConvertFilterToLinq(string filterString)
        {
            ...
            filterString = filterString.Replace("IS NULL", "== null");
            ...
            filterString = Regex.Replace(filterString,
                ...
                {
                    var f = m.Groups["field"].Value;
                    bool isString = m.Groups["vals"].Value.IndexOf("\'", StringComparison.OrdinalIgnoreCase) >= 0;
                    string[] vals;
                    if (isString) {
                        ...
                        string sql = $"{f}.Split(new[] {{ \"\\r\\n\",\"\\n\",\"\\r\" }},StringSplitOptions.RemoveEmptyEntries)";
                        sql = $"{sql}.Any(c=> (new[] {{ {joined} }}).Contains(c.Trim()))";
                        return sql;
                    }
                    else
                    {
        ...
        protected override void OnCellMouseMove(DataGridViewCellMouseEventArgs e)
        {
            ...
            if (e.RowIndex == -1 || e.ColumnIndex == -1) // header row or column header
            {
        ...
        protected override void OnColumnHeaderMouseClick(DataGridViewCellMouseEventArgs e)
        {
            ...
            if (this.IsSortHandledByChild)
            {
                this.IsSortHandledByChild = false; // Reset flag
                return;
            }
            var col = this.Columns[e.ColumnIndex];
            if (col == null)
            ...
            //// 個別Gridで指定した列ごとにDBソートする場合はそのイベントへ移動する
            //bool isSortByDB = OnSortByDatabase(col, propName, newDirection);
            //if (isSortByDB)
            //{
            //    return;
            //}
        ...
        //protected virtual bool OnSortByDatabase(DataGridViewColumn col, string propName, ListSortDirection direction)
        //{
        //    // 個別GridでOnSortByDatabase設定しない場合は現在DataSourceで並び替える
        //    return false;
        //}
        /// <summary>
        ...
        {
            var config = GetExternalConfig(e.ColumnIndex);
            if (config != null)
            {
                e.ThrowException = false;
                if (e.Exception is InvalidCastException || e.Exception is ArgumentException)
                {
                    this.CancelEdit();
                    e.Cancel = false;
                    return;
        ...
        protected override void OnCellParsing(DataGridViewCellParsingEventArgs e)
        {
            ...
            var config = GetExternalConfig(e.ColumnIndex);
            //特別設定しない場合は通常通りです。
            if (config == null) { base.OnCellParsing(e); return; }
            //入力した値が入力しなかった場合
            if (string.IsNullOrWhiteSpace(e.Value?.ToString()))
            {
                if (!config.AllowNullInput) return; // NULL入力不可ならばrollbackする
                // NullValueのセットある場合
                if (config.NullValue != null)
                {
                    Type targetType = this.Columns[e.ColumnIndex].ValueType;
                    e.Value = Convert.ChangeType(config.NullValue, Nullable.GetUnderlyingType(targetType) ?? targetType);
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            for (int i = 0; i < this.Columns.Count; i++)
            {
                ...
                bool isFilterDisabled = this.DisabledFilterAll || IsFilterDisabled(colName);
        ...
        protected override void OnCurrentCellDirtyStateChanged(EventArgs e)
        {
            ...
            // for checkbox
            bool isCheckBox = this.CurrentCell is DataGridViewCheckBoxCell || this.CurrentCell.OwningColumn is DataGridViewCheckBoxColumn;
            if (isCheckBox && this.IsCurrentCellDirty)
            {
                this.CommitEdit(DataGridViewDataErrorContexts.Commit);
            }
        }
        ...
        protected override void OnCellFormatting(DataGridViewCellFormattingEventArgs e)
        {
            ...
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                var config = GetExternalConfig(e.ColumnIndex);
                if (config != null && config.NullValue != null && e.Value != null && e.Value.Equals(config.NullValue))
                {
                    //日付列でNullValueの設定と一致する場合、空文字にする
        ...
        public bool IsFilterDisabled(string columnName)
        {
            return this.DisabledFilterColumns.Contains(columnName);
        }
        ...
        internal void AddNewRow()
        {
            ...
            //add to _originalListData
            //if (_originalListData == null) _originalListData = new List<object>();
            //var newItem = this.GetRow(newRowIndex);
            if (newItem != null)
            {
                AddItem(newItem);
            }
        ...
        private void ApplyListSortAsNumber(string propName, ListSortDirection direction)
        {
            ...
            try
            {
                ...
                if (this.DataSource is BindingSource bs)
                {
                    ...
                    bs.ResetBindings(false); //data更新のみ
                }
                else
        ...
        #region External Config for Custom Columns (e.g. DataGridViewTextBoxColumnEx)
        /// <summary>
        /// 
        /// </summary>
        /// <param name="columnName"></param>
        /// <param name="config"></param>
        // add external config custom column
        public void SetExternalColumnConfig(string columnName, ColumnConfig config)
        {
            if (this.Columns.Contains(columnName))
                _externalConfigs[columnName] = config;
        }

        // helper get external config column
        private ColumnConfig GetExternalConfig(int columnIndex)
        {
            var col = this.Columns[columnIndex];

            // 1. Custom column
            //if (col is DataGridViewTextBoxColumnEx colEx)
            //    return colEx.Config;

            // 2. nomal column get from Dictionary
            if (_externalConfigs.TryGetValue(col.Name, out ColumnConfig config))
                return config;

            return null;
        }

        public void SetExternalConfigDateColumnNullInput(params string[] columnNames)
        {
            ...
            {
                SetExternalColumnConfig(colName, config);
            }
            
        }

        public void SetExternalConfigLongColumnNullInput(params string[] columnNames)
        {
            var config = new ColumnConfig
            {
                NullValue = long.MinValue
            };
            foreach (string colName in columnNames)
            {
                SetExternalColumnConfig(colName, config);
        ...
        public void AutoConfigNullInputColumn()
        {
            foreach (DataGridViewColumn col in this.Columns)
            {
                if (GetExternalConfig(col.Index) == null)
                {
                    Type colType = col.ValueType;

                    if (colType == typeof(int) || colType == typeof(long) || colType == typeof(decimal) || colType == typeof(double))
                    {
                        this.SetExternalColumnConfig(col.Name, new ColumnConfig { RestoreOldValueOnInvalid = true });
                    }
                    else if (colType == typeof(DateTime) || col.Name.Contains("Date"))
                    {
                        this.SetExternalColumnConfig(col.Name, new ColumnConfig { NullValue = DateTime.MinValue });
                    }
                }
            }
        }
        #endregion External Config for Custom Columns
    }
}